    ...
    return (
        <div className='filter-option'>
            ...
            {options.map((option , index) => {
                return (
                    <>
                        <InputTag type="radio" className="mx-2 gender-option" name="radio2" value={option} key={index} onClick={()=>setFilterQuery(prevState => ({ ...prevState, ['Gender']: option.toLowerCase()}))}/>{option.charAt(0).toUpperCase() + option.slice(1)}<br />
                    </>
                )
            })
            }
